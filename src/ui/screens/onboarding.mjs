...
const STEPS = [
  ...
  },
  {
    ...
    visual: `<div class="restore-demo" aria-label="RestaMio sostituisce i segnaposto con i dati originali conservati sul telefono.">
      <div class="restore-demo__state restore-demo__state--protected"><span class="restore-demo__label"><svg class="restore-demo__icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M12 2 4 5v6c0 5 3.4 8.8 8 11 4.6-2.2 8-6 8-11V5l-8-3Z"/><rect x="9" y="10" width="6" height="5" rx="1"/><path d="M10.5 10V8.8a1.5 1.5 0 0 1 3 0V10"/></svg>Testo protetto</span><p>Ciao <mark>[NOME_1]</mark>, ti confermo il preventivo.<br>Scrivimi a <mark>[EMAIL_1]</mark></p></div>
      <svg class="restore-demo__arrow" aria-hidden="true" viewBox="0 0 24 24"><path d="M12 3v16m0 0-5-5m5 5 5-5"/></svg>
      <div class="restore-demo__state restore-demo__state--restored"><span class="restore-demo__label"><svg class="restore-demo__icon" aria-hidden="true" viewBox="0 0 24 24"><path d="M12 2 4 5v6c0 5 3.4 8.8 8 11 4.6-2.2 8-6 8-11V5l-8-3Z"/><path d="m8.5 12 2.2 2.2 4.8-5"/></svg>Testo ripristinato</span><p>Ciao <mark>Luca</mark>, ti confermo il preventivo.<br>Scrivimi a <mark>luca@email.it</mark></p></div>
    </div>`,
    cta: 'Iniziamo',
  },
];
...
export function renderOnboarding(step) {
  ...
  return `<section class="screen onboarding" data-onboarding-step="${step}">
    ...
    <div class="onboarding__content">
      ${item.titleFirst ? `<h1>${item.title}</h1><div class="onboarding__visual">${item.visual}</div>` : `<div class="onboarding__visual">${item.visual}</div><h1>${item.title}</h1>`}<p>${item.body}</p>
    </div>
