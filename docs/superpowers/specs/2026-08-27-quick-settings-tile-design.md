...
- **Tocchi ripetuti rapidi**: **corretto due volte.** Il ragionamento originale ("è idempotente, il secondo tocco non trova nulla da fare") era sbagliato: gli appunti appena mascherati contengono i NOSTRI segnaposto, quindi un secondo tocco li riconosce come una risposta da ripristinare e rimette il dato vero negli appunti — l'esatto contrario di quello che serve, proprio nel momento in cui l'utente sta per incollare in un'IA. Una prima correzione ha aggiunto solo una finestra di 3 secondi (`QUICK_PROTECT_DEBOUNCE_MS`), che sposta il problema invece di risolverlo (un tocco oltre i 3 secondi lo riapre). **Guardia definitiva**: prima di decidere, il testo negli appunti viene confrontato con l'ultimo testo che il riquadro stesso ha scritto (`quickProtectLastWritten`, salvato a ogni scrittura); se coincide byte per byte, non è una nuova risposta — un'IA vera non risponde mai in modo identico a quanto le è stato appena mandato — e il riquadro non fa nulla. La finestra di 3 secondi resta come guardia aggiuntiva, più grezza, contro due tocchi praticamente simultanei che avvierebbero due Activity concorrenti.
