...
Nessun relay cloud. Telefono e computer sulla stessa rete (o hotspot dal telefono); il QR contiene solo indirizzo locale + credenziali effimere, mai dati; originali/mapping/chiavi permanenti restano sul telefono; il computer riceve di default solo la versione protetta. Ordine originale: **APK reale → Keystore → Share Target nativo → threat review → protocollo locale cifrato → desktop QR.**

**Decisione presa il 28/08/2026, esplicita:** il "protocollo locale cifrato" NON è stato implementato come prerequisito bloccante — è stato riclassificato come miglioramento successivo, non come condizione per spedire il ponte. Motivo: cifrare il trasporto senza far installare nulla sul computer (il vincolo "solo browser, zero installazioni" è esplicito e voluto) è un lavoro non banale, e bloccare l'intera feature su quello avrebbe rimandato indefinitamente una funzione già richiesta. **Limite accettato consapevolmente, non scoperto per caso**: oggi il traffico fra telefono e computer (`/api/store/:key`, incluso il token di sessione e la mappa segnaposto→dato reale) viaggia in **HTTP semplice, senza cifratura di trasporto**, sulla rete Wi-Fi locale. Chiunque sia sulla stessa rete può leggerlo passivamente. Su una rete domestica fidata il rischio è basso; su una rete pubblica (bar, coworking, hotel, aeroporto) non lo è. Il modello di minaccia del ponte è quindi "rete fidata", non "rete qualsiasi" — e questo va detto all'utente, non solo scritto qui: un avviso in tal senso è nel dialogo "Continua sul computer" (`index.html`, `#dlg-desktop`). Se in futuro si vuole chiudere il divario senza installazioni, la strada più realistica è cifrare il corpo di `/api/store/:key` con una chiave derivata dal token (che viaggia già fuori banda via QR/link) — non risolve un replay ma toglie la lettura passiva.
